...
Last reconciled: **2026-04-28** (Phase-1 entry reconciliation pass).
...
| [W-10](W-10.md) | Medium | WebAuthn → secp256k1 path is fragile | 🟧 WebAuthn fallback shipped (`packages/security/src/fido.ts`); native libykcs11 helper deferred to M3-M4 council standup |
...
| [W-13](W-13.md) | Medium | Tor-exit fingerprinting risk unaddressed | 🟩 layered egress (`packages/adapters/src/proxy.ts`) |
| [W-14](W-14.md) | High | Anti-hallucination L1-L12 not testable | 🟧 12-layer guards live; corpus expanded 7→40 rows; L5 implemented; L8/L9/L10/L12 worker-level rows seeded (`packages/llm/__tests__/synthetic-hallucinations.jsonl`). Per-pattern expansion ongoing toward 200-row target. |
...
## Status tally (2026-04-28 reconciliation)

- 🟩 committed: 18
- 🟧 in progress: 2 (W-10 native helper, W-14 corpus expansion)
