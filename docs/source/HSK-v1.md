...
### 4.8 Council Vote Signing — Native PKCS#11 (DEFERRED to M3-M4)

**STATUS: deferred-to-M3 — WebAuthn fallback is the live path through M2; native PKCS#11 + secp256k1 path lands M3-M4 per W-10 / PHASE-1-COMPLETION.md §D5. Do NOT remove this banner without an architect signoff.**

Mechanism (planned, M3-M4)
Each council member signs a YES/NO/ABSTAIN/RECUSE vote on a finding's proposal hash before the vote is submitted on-chain via vigil-polygon-signer. The native path uses libykcs11 + a CLI helper (`vigil-vote-sign`) that drives the YubiKey's PIV slot 9D (Card Authentication) over PKCS#11 to produce a secp256k1 signature over `keccak256(proposalIndex || choice || nonce)`. The signature is bundled with the vote and the on-chain `vote()` call verifies it against the council member's enrolled address.

Live path through M2 (WebAuthn fallback)
The dashboard's `/council/vote/:proposalId` page invokes `navigator.credentials.get(...)` against the council member's WebAuthn FIDO2 credential. The browser produces a CBOR-shaped attestation that the dashboard server verifies against the stored credential public key. Verified votes are then submitted on-chain via vigil-polygon-signer, signed by the architect's PIV slot 9C (the existing Polygon wallet path in §4.7). The council member's individual cryptographic ownership of the vote is asserted at the WebAuthn layer; the on-chain transaction itself is architect-signed.

Why the deferral
The WebAuthn path is web-native, requires no client-side software install, and works with the same FIDO2 applet enrolled at provisioning (§5.7). The native PKCS#11 path requires each council member to install yubikey-manager + a CLI helper on their workstation and learn a non-trivial workflow. Phase-1 institutional priority is shipping the council ceremony at all; the native path is a hardening upgrade scheduled for after Phase-2 dossier-delivery rolls out (M3-M4 per ROADMAP.md).

What Fails Without A Key
The same as §4.6 (FIDO2): the council member cannot sign votes. WebAuthn fallback assumes both YubiKeys still hold the FIDO2 credential; loss of both blocks vote-signing until re-enrolment via the §3.4 backup-becomes-primary procedure.

Break-Glass Recovery
A council member who has lost both keys re-enrols via an in-person session with the architect at the secure room. Until re-enrolment completes the member's pillar is treated as RECUSED for any active proposals, and the quorum math (3-of-5 over non-recused) accommodates the absence per SRD §23.
