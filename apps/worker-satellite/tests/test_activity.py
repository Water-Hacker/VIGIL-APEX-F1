...
def test_no_change_yields_low_activity() -> None:
    ...
    swir = _flat(0.20)
    r = compute_activity(
        before_red=red, before_nir=nir, before_swir=swir,
        after_red=red, after_nir=nir, after_swir=swir,
    )
...
def test_clear_construction_signal() -> None:
    ...
    r = compute_activity(
        before_red=before_red, before_nir=before_nir, before_swir=before_swir,
        after_red=after_red, after_nir=after_nir, after_swir=after_swir,
    )
...
def test_partial_construction_localised_centroid() -> None:
    ...
    # Build a 10×10 patch in the SE corner
...
def test_shape_mismatch_raises() -> None:
    ...
    with pytest.raises(VigilError) as exc:
        compute_activity(
            before_red=a, before_nir=a, before_swir=a,
            after_red=b, after_nir=b, after_swir=b,
        )
...
def test_all_nan_raises() -> None:
    ...
    with pytest.raises(VigilError) as exc:
        compute_activity(
            before_red=nan, before_nir=nan, before_swir=nan,
            after_red=nan, after_nir=nan, after_swir=nan,
        )
