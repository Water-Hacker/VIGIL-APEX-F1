...
describe('worker-tip-triage — SafeLlmRouter migration (Block-B A2)', () => {
  ...
  it('tip body is passed as a closed-context source (NOT inlined into the user prompt)', () => {
    ...
    expect(INDEX_TS).toMatch(/sources:\s*\[\s*\{[^}]*id:\s*`tip:\$\{tip\.id\}`/);
    expect(INDEX_TS).toMatch(/text:\s*text\.slice\(0,\s*4000\)/);
  });

  it('schema validation via zParaphrase is preserved (L5)', () => {
    expect(INDEX_TS).toMatch(/responseSchema:\s*zParaphrase/);
  });
  ...
  it('TIP_PARAPHRASE_TASK retains the PII-stripping rules (saliency check)', () => {
    ...
    expect(PROMPTS_TS).toMatch(/Strip personally identifying detail/);
    expect(PROMPTS_TS).toMatch(/max 500 characters/);
    expect(PROMPTS_TS).toMatch(/insufficient evidence/);
  });
});
